...
const removeConfig = t => (path, componentId) => {
  if (
    path.node.callee &&
    t.isMemberExpression(path.node.callee.callee) &&
    path.node.callee.callee.property &&
    ...
    path.node.callee.arguments[0].properties =
      path.node.callee.arguments[0].properties.filter(
        prop => prop.key.name !== 'displayName'
      )
    return
  }
}
...
export default t => (path, state) => {
  ...
  if (
    ...
    useForceRemoveDisplayName(state)
  ) {
    removeConfig(t)(path, useSSR(state) && getComponentId(state))
  }
}
